...
drop policy if exists pr_own on proposals;
...
drop policy if exists pv_own on proposal_versions;
...
drop policy if exists c_own on communications;
...
drop policy if exists f_own on follow_ups;
...
drop policy if exists bs_own on business_analyses;
...
drop policy if exists ps_own on proposal_shares;
...
drop policy if exists pe_own on proposal_events;
...
drop policy if exists ar_own on ai_recommendations;
...
-- After running this migration (e.g. via the Supabase SQL editor), PostgREST's
-- schema cache can be stale until it reloads, which surfaces in the app as
-- "Could not find the table 'public.prospects' in the schema cache". This
-- forces an immediate reload so the API reflects the tables/policies above
-- without waiting for the next automatic refresh.
