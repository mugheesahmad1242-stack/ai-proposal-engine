...
export async function POST(req: Request) {
  const { supabase, user, response } = await requireUser(); if (!user) return response!;
  const parsed = await parseJson(req, schema); if (parsed.error) return parsed.error; const b = parsed.data;
  ...
  if (b.urls.length) {
    ...
    if (sourceError) { await supabase.from('prospects').delete().eq('id', p.id).eq('user_id', user.id); return NextResponse.json({ error: sourceError.message }, { status: 400 }); }
  }
