...
export async function GET(_r: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const pdf = await PDFDocument.create(); let page = pdf.addPage([595, 842]); const font = await pdf.embedFont(StandardFonts.Helvetica); let y = 790; const maxWidth = 505;
  const wrap = (txt: string, size: number) => {
    const lines: string[] = [];
    for (const raw of txt.split('\n')) {
      const words = raw.split(/\s+/).filter(Boolean); let cur = '';
      for (const w of words) {
        const candidate = cur ? `${cur} ${w}` : w;
        if (font.widthOfTextAtSize(candidate, size) > maxWidth && cur) { lines.push(cur); cur = w; } else cur = candidate;
      }
      lines.push(cur);
    }
    return lines;
  };
  const draw = (txt: string, size = 11) => { for (const line of wrap(txt, size)) { if (y < 50) { page = pdf.addPage([595, 842]); y = 790; } page.drawText(line, { x: 45, y, size, font, color: rgb(.1, .1, .12) }); y -= size + 8; } };
