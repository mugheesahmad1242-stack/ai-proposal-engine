...
export default function Dashboard() {
  const [prospects,setProspects] = useState<Prospect[]>([]); const [analytics,setAnalytics] = useState<Analytics>(); const [loading,setLoading] = useState(true); const [error,setError] = useState('');
  useEffect(() => { Promise.all([fetch('/api/prospects').then(r=>r.json()), fetch('/api/analytics').then(r=>r.json())]).then(([p,a])=>{setProspects(p.prospects||[]);setAnalytics(a)}).catch(()=>setError('Could not load your workspace data. Check your connection and try again.')).finally(()=>setLoading(false)); },[]);
